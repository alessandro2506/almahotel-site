...
| 2026-05-14 | Claude / Alvenco | v2.3 — i18n audit approfondito (round 2). Fix conflitto `generateMetadata` + `use client` in `web-check-in/page.tsx`: rimossa direttiva `'use client'` (next-intl v3 supporta `useTranslations` in Server Components). Audit completo testi sopra immagini e attributi accessibilità: (1) Metadata `title`/`description` delle 3 pagine camere ora locale-aware via `t('metaTitle')` e `t('metaDescription')` nei namespace `suite`, `matrimoniale`, `matrimonialeSuperiore`. (2) `Navbar.tsx`: `aria-label` open/close menu ora usa `t('openMenu')` e `t('closeMenu')` dal namespace `nav`. (3) `camere/page.tsx`: `alt` immagine hero ora usa `t('title')`. (4) `RoomDetail.tsx`: `alt` gallery rimossi i termini italiani `principale`/`dettaglio`. Aggiunte chiavi: `nav.openMenu`, `nav.closeMenu`, `suite.metaTitle`, `suite.metaDescription`, `matrimoniale.metaTitle`, `matrimoniale.metaDescription`, `matrimonialeSuperiore.metaTitle`, `matrimonialeSuperiore.metaDescription` in tutti e 4 i file JSON. |
| 2026-05-14 | Claude / Alvenco | v2.4 — i18n completa al 100%. Tradotti tutti i restanti attributi `alt` delle immagini nei componenti homepage e nella pagina Chi Siamo. Nomi propri invariabili conservati (Palermo, Alma Hotel, Sapori Perduti, Mondello). Componenti aggiornati: `RestaurantSection.tsx` (`restaurant.imageAlt`), `ExperienceSection.tsx` (`experience.imageAlt`), `HighlightBanner.tsx` (`highlight.imageAlt`), `chi-siamo/page.tsx` (`about.heroAlt`, `about.historyImageAlt`, `about.teamImageAlt`). Chiavi aggiunte in tutti e 4 i JSON. Commit: 30c1e66. |
